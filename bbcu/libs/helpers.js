	...
	async start () {
		...
		return new Promise((resolve, reject) => {
			async.retry(retry_opts, async () => {
				...
				logger.debug(`Checking status of request, type: ${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]}, nonce: ${qr_code_nonce}, step_number ${step_number}. Attempt ${++attempts}/${retry_opts.times}`);
				let queryObj = {};
				...
				if (!updated_request || (updated_request.length > 0 && !updated_request[0].state)) {
					throw new Error(`${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]} state could not be determined`);
					...
					if ((type & InboundNonceWatcher.REQUEST_TYPES.CONNECTION && [ 'connected' ].indexOf(updated_request[0].state) >= 0) ||
						...
						throw new Error(`${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]} with nonce ${qr_code_nonce} is in an unexpected state`);
					}
				} else {
					throw new Error(`Still waiting on ${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]} to be complete`);
				}
			}, (error, found_request) => {
				if (error) {
					logger.error(`Failed to establish ${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]} with nonce ${qr_code_nonce}: ${error}`);
					return reject(new Error(`${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]} with nonce ${qr_code_nonce} failed: ${error}`));
				}
				...
				logger.info(`${InboundNonceWatcher.REQUEST_TYPES_KEYS_AS_STRINGS[type & step_number]} with nonce ${qr_code_nonce} successfully established with agent ${agent_did}`);
