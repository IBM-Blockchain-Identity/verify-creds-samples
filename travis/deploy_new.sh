...
sed -i "s+ACCOUNT_URL_HERE+${NEW_ACCOUNT_URL}+g" $DEPLOYMENT_SPEC

sed -i "s+DMV_AGENT_ID_HERE+$NEW_DMV_AGENT_ID+g" $DEPLOYMENT_SPEC
sed -i "s+DMV_AGENT_NAME_HERE+$NEW_DMV_AGENT_NAME+g" $DEPLOYMENT_SPEC
sed -i "s+DMV_AGENT_PASSWORD_HERE+$NEW_DMV_AGENT_PASSWORD+g" $DEPLOYMENT_SPEC

sed -i "s+IBMHR_AGENT_ID_HERE+$NEW_IBMHR_AGENT_ID+g" $DEPLOYMENT_SPEC
sed -i "s+IBMHR_AGENT_NAME_HERE+$NEW_IBMHR_AGENT_NAME+g" $DEPLOYMENT_SPEC
sed -i "s+IBMHR_AGENT_PASSWORD_HERE+$NEW_IBMHR_AGENT_PASSWORD+g" $DEPLOYMENT_SPEC

sed -i "s+BBCU_AGENT_ID_HERE+$NEW_BBCU_AGENT_ID+g" $DEPLOYMENT_SPEC
sed -i "s+BBCU_AGENT_NAME_HERE+$NEW_BBCU_AGENT_NAME+g" $DEPLOYMENT_SPEC
sed -i "s+BBCU_AGENT_PASSWORD_HERE+$NEW_BBCU_AGENT_PASSWORD+g" $DEPLOYMENT_SPEC

sed -i "s+HR_AGENT_DID_HERE+$NEW_IBMHR_AGENT_DID+g" $DEPLOYMENT_SPEC
sed -i "s+DMV_AGENT_DID_HERE+$NEW_DMV_AGENT_DID+g" $DEPLOYMENT_SPEC

sed -i "s+COUCHDB_USER_NAME_HERE+${NEW_COUCHDB_USER_NAME}+g" $DEPLOYMENT_SPEC
sed -i "s+COUCHDB_USER_PASSWORD_HERE+${NEW_COUCHDB_USER_PASSWORD}+g" $DEPLOYMENT_SPEC
...
ibmcloud ks cluster config --cluster ${NEW_IBMCLOUD_DEPLOYMENT_CLUSTER}
